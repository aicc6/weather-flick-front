...
const TEST_REGIONS = [
  ...
  { code: 'gangwon_taebaek', name: '태백시', province: 'gangwon', type: 'city' }
];
...
function checkRegionSupport(regionIdentifier) {
  ...
  try {
    // 정확한 매칭 시도
    let matchedRegion = TEST_REGIONS.find(region => 
      region.code === trimmedInput ||
      region.name === trimmedInput
    );
    ...
    // 부분 매칭 시도
    const similarRegions = TEST_REGIONS.filter(region =>
...
async function runEdgeCaseTests() {
  ...
  invalidInputs.forEach(test => {
    ...
    console.log(`${test.description}: ${result.isSupported ? '❌ 잘못 처리됨' : '✅ 올바르게 처리됨'}`);
    if (result.error) {
  ...
  specialCharInputs.forEach(input => {
    ...
    console.log(`"${input}": ${result.isSupported ? '✅ 지원' : '❌ 미지원'} | 매핑: ${result.regionName || 'N/A'}`);
  });
  ...
  lengthTests.forEach(test => {
    ...
    console.log(`${test.description}: ${result.isSupported ? '지원' : '미지원'} (${duration.toFixed(2)}ms)`);
    if (duration > 10) {
  ...
  console.log(`존재하지 않는 키: ${nonExistent ? '❌ 잘못 반환' : '✅ null 반환'}`);
  ...
  asyncResults.forEach(result => {
    console.log(`지역 "${result.region}": ${result.correct ? '✅ 예상대로' : '❌ 예상과 다름'}`);
  ...
  console.log(`동시 요청 10개: ${successCount}개 성공 (${(endTime - startTime).toFixed(2)}ms)`);
  ...
  const heavyLoadPromises = Array(100).fill().map(() => 
    checkRegionSupport('제주')
  );
  ...
  console.log(`100개 동시 요청: ${allSucceeded ? '✅ 모두 성공' : '❌ 일부 실패'} (${(heavyLoadEnd - heavyLoadStart).toFixed(2)}ms)`);
