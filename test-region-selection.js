...
function runRegionSelectionTests() {
  ...
  console.log(
    '다양성:',
    scenario1.usedCategories.length === scenario1.selectedRegions.length
      ? '✅ 완벽'
      : '⚠️ 부분적',
  )
  ...
  Object.entries(categoryStats).forEach(([category, regions]) => {
    const avgPriority = (
      regions.reduce((sum, r) => sum + r.priority, 0) / regions.length
    ).toFixed(1)
    console.log(
      `${category}: ${regions.length}개 지역, 평균 우선순위 ${avgPriority}`,
    )
    console.log(
      `  대표 지역: ${regions
        .slice(0, 3)
  ...
  const priorityGroups = {
    ...
    '높은 우선순위 (6-7)': PRIORITY_REGIONS.filter(
      (r) => r.priority >= 6 && r.priority < 8,
    ),
    '중간 우선순위 (4-5)': PRIORITY_REGIONS.filter(
      (r) => r.priority >= 4 && r.priority < 6,
    ),
  ...
  const performanceTests = [
    ...
    {
      existing: new Set(PRIORITY_REGIONS.slice(0, 20).map((r) => r.name)),
      max: 3,
      name: '대부분 존재',
    },
  ]
  ...
  performanceTests.forEach((test) => {
    ...
    console.log(
      `${test.name}: ${result.selectedRegions.length}개 선택 (${(endTime - startTime).toFixed(2)}ms)`,
    )
