...
import { getAllRegionsFlat } from './src/data/koreaRegions.js'
import { checkRegionSupport } from './src/services/dynamicRegionService.js'
...
async function testRegionCoverage() {
  ...
  for (const region of testRegions) {
    ...
    try {
      ...
      console.log(
        `  ✅ 지원 여부: ${supportInfo.isSupported ? '지원' : '미지원'}`,
      )
      ...
      if (supportInfo.isSupported) {
        ...
        console.log(
          `  💡 추천: ${supportInfo.suggestions.map((s) => s.name).join(', ')}`,
        )
  ...
  console.log(
    `성공: ${successCount}개 (${((successCount / testResults.length) * 100).toFixed(1)}%)`,
  )
  console.log(
    `실패: ${failedCount}개 (${((failedCount / testResults.length) * 100).toFixed(1)}%)`,
  )
  console.log(
    `오류: ${errorCount}개 (${((errorCount / testResults.length) * 100).toFixed(1)}%)`,
  )
  console.log(
    `정확 매칭: ${exactCount}개 (${((exactCount / testResults.length) * 100).toFixed(1)}%)`,
  )
  ...
  specialCases.forEach((testCase) => {
    ...
    console.log(
      `  ${testCase.input} → ${result.regionName || '미지원'} ${success ? '✅' : '❌'}`,
    )
  })
