...
import { generateTravelCourse, generateMultipleCourses } from '@/services/tmapCourseService'
...
export default function TmapCourseStatus() {
  ...
  const checkApiAvailability = async () => {
    try {
      ...
      setStatus(prev => ({ ...prev, apiAvailable: false, error: error.message }))
    }
  }
  ...
  const generateMultipleTestCourses = async () => {
    ...
    try {
      ...
      for (const region of testRegions) {
        try {
          setStatus(prev => ({ ...prev, progress: (completedCount / totalRegions) * 100 }))
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          Tmap 여행 코스 생성 상태
          <Badge 
            variant={status.apiAvailable ? 'default' : 'destructive'}
          >
      ...
      <CardContent className="space-y-6">
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {status.generatedCourses.map((course, index) => (
                <div key={index} className="p-2 bg-muted rounded text-sm">
                  ...
                  <div className="text-muted-foreground">
                    {course.region} • {course.duration} • {course.itinerary?.length || 0}일 일정
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">
                    POI: {course.itinerary?.reduce((sum, day) => sum + (day.places?.length || 0), 0) || 0}개 | 
                    평점: {course.rating} | 
                    테마: {course.theme?.join(', ') || 'N/A'}
                  </div>
                </div>
              ))}
            </div>
        ...
        {status.generatedCourses.length > 0 && (
          <div className="p-3 bg-primary/5 border border-primary/20 rounded-md">
            <h5 className="font-medium text-sm mb-2">생성 통계</h5>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                평균 POI 수: {Math.round(
                  status.generatedCourses.reduce((sum, course) => 
                    sum + (course.itinerary?.reduce((daySum, day) => 
                      daySum + (day.places?.length || 0), 0) || 0), 0
