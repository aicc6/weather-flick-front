...
import { Route, Clock, Navigation, Zap } from '@/components/icons'
  ...
  const openDayRoute = async (dayPlaces) => {
    ...
    try {
      ...
      if (validPlaces.length === 0) {
        ...
        const searchablePlaces = dayPlaces.filter(
          (place) => place.place_id || place.description,
        )
        ...
        if (searchablePlaces.length > 0) {
          ...
          const searchQuery = encodeURIComponent(
            firstPlace.description || firstPlace.name || '',
          )
    ...
    try {
      ...
      const selectedPlaces =
        allPlaces.length > 10
          ? allPlaces
              .filter(
                (_, index) => index % Math.ceil(allPlaces.length / 10) === 0,
              )
              .slice(0, 10)
          : allPlaces
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
          ...
          <ul className="mt-1 space-y-1 text-xs text-blue-700">
            <li>
              • <strong>개별 길찾기</strong>: 각 장소별로 최적 경로 제공
            </li>
            <li>
              • <strong>일차별 경로</strong>: 해당 일차의 모든 장소를 순서대로
              연결
            </li>
            <li>
              • <strong>전체 경로</strong>: 전 일정을 하나의 경로로 생성 (최대
              10곳)
            </li>
          </ul>
        </div>
        ...
        {days.map((day) => {
          ...
          return (
            <div key={day} className="rounded-lg border bg-gray-50 p-4">
        ...
        <div className="rounded-lg border bg-gray-50 p-3">
          <div className="text-sm text-gray-700">
            <div className="mb-1 font-medium">📋 경로 생성 정보</div>
