...
} from '@/utils/regionImageUtils.js'
...
export const searchRegionPOIs = async (regionName, theme = 'all') => {
  try {
    ...
    // 4. POI 데이터 정규화
    const normalizedPOIs = categorizedPOIs.map((poi) =>
      normalizePOIData(poi, regionName),
    )
...
const categorizePOIsByTheme = (pois, theme) => {
  ...
  return pois.filter((poi) => {
    const searchText =
      `${poi.name} ${poi.bizCatTcode} ${poi.middleBizCateNm}`.toLowerCase()
  ...
  const fallbackPOIs = [
    {
      ...
      coordinates: { lat: 37.5665, lng: 126.978 }, // 기본 좌표 (서울)
      category: '관광',
...
export const generateTravelCourse = async (regionName, options = {}) => {
  const { theme = 'all', duration = '2박 3일', difficulty = 'easy' } = options
...
const generateCourseImages = (regionName, pois) => {
  ...
  pois.slice(0, 3).forEach((poi, index) => {
    images.push(
      `https://source.unsplash.com/800x600/?${encodeURIComponent(poi.name)},korea`,
    )
  })
