...
export const getApiUsage = (apiName = null, date = null) => {
  try {
    const usage = JSON.parse(
      localStorage.getItem(STORAGE_KEYS.API_USAGE) || '{}',
    )
...
export const getApiStatusReport = () => {
  ...
  Object.entries(API_KEYS_INFO).forEach(([key, info]) => {
    ...
    if (
      todayUsage > info.dailyLimit * 0.7 ||
      monthlyUsage > info.monthlyLimit * 0.7
    ) {
...
const showUsageAlert = (apiName, type, usage, limit) => {
  ...
  const alerts = JSON.parse(
    localStorage.getItem(STORAGE_KEYS.API_ALERTS) || '[]',
  )
  if (alerts.includes(alertKey)) return

  console.warn(`⚠️ API 사용량 경고: ${API_KEYS_INFO[apiName]?.name}`)
  console.warn(
    `${type === 'daily' ? '일일' : '월간'} 사용량: ${usage}/${limit} (${percentage}%)`,
  )
    ...
    if (expiringKeys.length > 0) {
      ...
      expiringKeys.forEach((key) => {
        if (key.expired) {
          ...
          console.warn(
            `🕐 ${key.name}: ${key.daysUntilExpiry}일 후 만료 (${key.expiryDate})`,
          )
        }
      })
